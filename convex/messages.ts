...
const poplulateReactions = (ctx: QueryCtx, messageId: Id<"messages">) => {
            ...
            page: (
                await Promise.all(
                    result.page.map(async (message)=>{
                        ...
                        const user = member? await populateUser(ctx, userId): null;
                        if(!member || !user) {
                            return null;
                        }
                        const reactions = await poplulateReactions(ctx, message._id);
