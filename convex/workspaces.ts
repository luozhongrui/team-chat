...
export const join = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const existingmemeber = await ctx.db.query("members").withIndex("by_user_id_and_workspace_id", (q)=>q.eq("userId", userId).eq("workspaceId", args.workspaceId)).unique();
    if(existingmemeber){
